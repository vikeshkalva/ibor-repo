...
with current_positions as (
    select * from CAPSTONE_PRJ.PUBLIC_SILVER.int_current_positions
),
...
securities as (
    select
        ...
        sector,
        country
    from CAPSTONE_PRJ.PUBLIC_RAW_DATA.Securities_MASTER
),
...
portfolios as (
    select
        ...
        portfolio_manager,
        base_currency
    from CAPSTONE_PRJ.PUBLIC_RAW_DATA.portfolios
),
...
fx_rates as (
    select
        ...
        to_currency,
        exchange_rate
    from CAPSTONE_PRJ.PUBLIC_RAW_DATA.fx_rates
    where rate_date = (select max(rate_date) from CAPSTONE_PRJ.PUBLIC_RAW_DATA.fx_rates)
),
